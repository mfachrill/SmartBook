    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-4">
            <div class="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
                <form method="POST" action="{{ route('books.my.update', $book) }}" enctype="multipart/form-data" class="p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <textarea id="description" name="description" rows="6"
                                        class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm">{{ old('description', $book->description) }}</textarea>
                        ...
                        <div class="space-y-6">
                            <div class="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                                <div id="imagePreview" class="{{ $book->thumbnail || old('thumbnail') ? '' : 'hidden' }}">
                                    <img id="previewImage" class="mx-auto h-64 w-auto object-contain rounded" 
                                        src="{{ $book->thumbnail ? asset('storage/' . $book->thumbnail) : (old('thumbnail') ? '#' : '') }}" 
                                        alt="Preview cover buku" />
                                </div>
                                <div id="emptyState" class="{{ $book->thumbnail || old('thumbnail') ? 'hidden' : 'flex flex-col items-center justify-center h-64 text-gray-400' }}">
                            ...
                            <div>
                                ...
                                <div class="mt-2 text-center">
                                    <p id="fileName" class="text-sm text-gray-500">
                                        {{ $book->thumbnail ? basename($book->thumbnail) : 'Belum ada file dipilih' }}
                                    </p>
    ...
    <script>
        document.getElementById('thumbnail').addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                fileName.textContent = '{{ $book->thumbnail ? basename($book->thumbnail) : 'Belum ada file dipilih' }}';
            }
        });
    </script>
</x-app-layout>